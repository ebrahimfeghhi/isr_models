...
Code is from https://github.com/qihongl/learn-hippo/blob/master/src/models/EM.py
and is written by Qihong Lu. 
"""
import torch
import torch.nn.functional as F
from copy import deepcopy
from LCA_pytorch import LCA
...
class EM():
    ...
    def get_memory(
        ...
        if len(self.vals) == 0 or self.retrieval_off:
            return dummy_memory(self.dim), dummy_memory(self.size)
    ...
    def _get_memory(
        ...
        # pass similarity to LCA to compute weights for each memory 
        w = lca_transform(
