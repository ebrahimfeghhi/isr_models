...
save_model_path = '/home3/ebrahim/isr/BP06/saved_models/'
...
parser.add_argument('--plastic', type=bool, default=False,
                    help='If true, incorporate hebbian plasticity')
...
parser.add_argument('--weight_decay', type=float, default=0,
                    help='L2 reg')
parser.add_argument('--h2h_weights', type=bool, default=True,
                    help='If true, bptt non-hebbian weights are allowed.')
parser.add_argument('--double_trial', type=bool, default=False,
                    help='If true, one trial contains two lists.')
parser.add_argument('--storage_frac', type=float, default=0,
                    help='Percentage of storage trials during training')
parser.add_argument('--delay_storage', type=int, default=0,
                    help='Amount of delay for storage trials')
...
def train_loop(settings, checkpoint_epoch=10000):
    ...
    run.name = f'stateful_{run.config.stateful}_noise_{run.config.noise_std}_opt_{run.config.opt}_\
    {run.config.grad_mode}_num_letters_{run.config.num_letters}_nl_test_{run.config.num_letters_test}_\
    {run.config.h2h_weights}_{run.config.run_number}'
    ...
    trainData = OneHotLetters(wandb.config['max_length'], wandb.config['num_cycles']+1, 
                                ...
                                delay_storage=wandb.config['delay_storage'], 
                                storage_frac=wandb.config['storage_frac'])
    ...
    wandb.config['bias'], wandb.config['nonlin'], wandb.config['noise_std'], wandb.config['plastic'], wandb.config['alpha_s'], 
    wandb.config['rule'], wandb.config['h2h_weights'])
...
settings = {
    ...
    'h2h_weights': args.h2h_weights,
    'storage_frac': args.storage_frac,
